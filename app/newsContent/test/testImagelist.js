...
describe('prepro',function(){
    describe('#prepro_imagelist',function(){
        it('the type of data-ad-type=COPYRIGHT_STRUCT should be COPY_RIGHT',function(done){
            fs.readFile('./app/tcwise/test/data/imagelist.txt', function (err, block) {
                if (err) throw err;
                //console.log(data);
                var res = {
              ...
              //console.log(block);
                ...
                predata.prepro_imagelist(block, res ,function(err, data){
                   //console.log(res);
                    ...
                    //res.page_ad += "</div>";
                    assert.equal('BANNER',res.BANNER.type);
                    assert.equal(1,res.BANNER.data_value.items.length);
                    assert.equal('noTitle',res.BANNER.data_value.no_title);
                    done();
        ...
        it('testImagelist block signtype must be no contain IGNORE',function(done){
            fs.readFile('./app/tcwise/test/data/imagelist_1.txt', function (err, block) {
                if (err) throw err;
                //console.log(data);
                var res = {
                    'page_ad' : "<div class='ssNormal ssAD'>"
                ...
                predata.prepro_imagelist(block, res ,function(err, data){
                    ...
                    var test=signtype.indexOf("IGNORE_");
                    assert.equal(-1,test);
                    done();
