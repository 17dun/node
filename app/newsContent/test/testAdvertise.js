...
describe('prepro',function(){
    describe('#prepro_advertise',function(){
        it('the pos of data-ad-type=ADVERTISE should be 32',function(done){


            fs.readFile('./app/tcwise/test/data/advertise.txt', function (err, block) {
                if (err) throw err;
                //console.log(data);
                var res = {
              ...
              //  console.log(JSON.parse(block));
                ...
                cpredata.prepro_advertise(block, res ,function(err, data){
                   // console.log(res);
                    ...
                    res.page_ad += "</div>";
                    assert.equal(32,res.page_ad.search(/data-ad-type=ADVERTISE/));
                    done();
