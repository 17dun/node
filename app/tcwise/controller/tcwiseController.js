...
function page_data(req,res,options,callback){
    ...
    //建站平台的自有网站名
    if(req.params.join().match(/jianzhan/)){
        var data = {};
        options.time_record['site_name']['last'] = +new Date() - site_name_startt;
        data.site_name = JSON.parse(req.body.siteInfo).site_title ? JSON.parse(req.body.siteInfo).site_title : "";
        data.site_url = req.body.url;
...
function show_page(data,req, res, options, callback){
    ...
    if(site_name_flag && pro_adv_info_flag){
        ...
        //jianzhan
        if(req.params.join().match(/jianzhan/)){
            tpl_data.jzFlag = true;
            tpl_data.jzInfo = JSON.parse(req.body.siteInfo);
            tpl_data.jzAdv = req.body.advInfo ? JSON.parse(req.body.advInfo) : false;
            tpl_data.statSiteId = req.body.stat_site_id;
            tpl_data.appid = req.body.appid;
            tpl_data.jzQuerysrc = req.body.querySrc;
            tpl_data.querySrc = req.body.querySrc;
        }else{
            tpl_data.jzFlag = false;
        }
        //处理footer数据
        tpl_data.pageFooter = footer(tpl_data.src, tpl_data.hostClaim, tpl_data.showClaim, tpl_data.jzFlag);
